...
Q_LOGGING_CATEGORY(ZIP_LOG, "ZIP")
...
{
    qCDebug(ZIP_LOG) << "Starting ZIP extraction";
    qCDebug(ZIP_LOG) << "ZIP file:" << m_zipFile->fileName();
    qCDebug(ZIP_LOG) << "Target directory:" << m_targetDir.absolutePath();

    if (!m_zipFile->open(QIODevice::ReadOnly))
    {
        qCDebug(ZIP_LOG) << "Failed to open ZIP file:" << m_zipFile->errorString();
        setError(BackendError::DiskError, m_zipFile->errorString());
        return;
    }

    qCDebug(ZIP_LOG) << "ZIP file opened successfully, size:" << m_zipFile->size();

    QString err;
    if (!extractAll(m_zipFile, m_targetDir, err))
    {
        qCDebug(ZIP_LOG) << "Extraction failed:" << err;
        setError(BackendError::DataError, err);
    }
    else
    {
        qCDebug(ZIP_LOG) << "Extraction completed successfully";
    }
    m_zipFile->close();
}
...
static quint16 rd16(QIODevice *in)
{
    quint16 v;
    in->read(reinterpret_cast<char *>(&v), 2);
    return qFromLittleEndian(v);
}
static quint32 rd32(QIODevice *in)
{
    quint32 v;
...
static inline quint16 rd16buf(const char *p)
{
    quint16 v;
    memcpy(&v, p, 2);
    return qFromLittleEndian(v);
}
static inline quint32 rd32buf(const char *p)
{
    quint32 v;
    memcpy(&v, p, 4);
    return qFromLittleEndian(v);
...
{
    qCDebug(ZIP_LOG) << "Starting extractAll";

    if (in->isSequential())
    {
        errorString = QStringLiteral("ZIP device must be seekable");
        return false;
    }
    ...
    qCDebug(ZIP_LOG) << "File size:" << fileSize;

    if (fileSize < 22)
    {
        errorString = QStringLiteral("ZIP too small");
        return false;
    }
    const qint64 searchLen = qMin<qint64>(fileSize, 65557); // 64K comment + EOCD
    qCDebug(ZIP_LOG) << "Search length:" << searchLen;

    if (!in->seek(fileSize - searchLen))
    {
        errorString = QStringLiteral("seek failed");
        return false;
    }
    const QByteArray tail = in->read(searchLen);
    if (tail.size() != searchLen)
    {
        errorString = QStringLiteral("read tail failed");
        return false;
    }

    qCDebug(ZIP_LOG) << "Read tail bytes:" << tail.size();

    int eocdIndex = -1;
    for (int i = tail.size() - 22; i >= 0; --i)
    {
        quint32 sig = rd32buf(tail.constData() + i);
        qCDebug(ZIP_LOG) << "Checking signature at offset" << i << ":" << QString::number(sig, 16);
        if (sig == SIG_EOCD)
        {
            eocdIndex = i;
            qCDebug(ZIP_LOG) << "Found EOCD at index:" << eocdIndex;
            break;
        }
    }
    if (eocdIndex < 0)
    {
        qCDebug(ZIP_LOG) << "EOCD not found, checking first few bytes";
        // Debug: show first few bytes
        for (int i = 0; i < qMin(20, tail.size()); i++)
        {
            qCDebug(ZIP_LOG) << "Byte" << i << ":" << QString::number((unsigned char)tail[i], 16);
        }
    ...
    qCDebug(ZIP_LOG) << "Total entries:" << totalEntries;
    qCDebug(ZIP_LOG) << "CD size:" << cdSize;
    qCDebug(ZIP_LOG) << "CD offset:" << cdOffset;

    if (!in->seek(cdOffset))
    {
        errorString = QStringLiteral("seek CD failed");
        return false;
    }
    ...
    for (quint16 i = 0; i < totalEntries; ++i)
    {
        ...
        {
            qCDebug(ZIP_LOG) << "Failed to seek to position" << currentPos;
            break;
        }
        ...
        {
            qCDebug(ZIP_LOG) << "Failed to read header at entry" << i;
            break;
        }

        quint32 sig = rd32buf(header);
        qCDebug(ZIP_LOG) << "Entry" << i << "signature:" << QString::number(sig, 16) << "expected:" << QString::number(SIG_CDH, 16);

        if (sig != SIG_CDH)
        {
            qCDebug(ZIP_LOG) << "Invalid signature at entry" << i << "- stopping parsing";
            break;
        }
        ...
        qCDebug(ZIP_LOG) << "Processing entry" << i << "nameLen:" << nameLen << "extraLen:" << extraLen << "commentLen:" << commentLen;

        // Read name/extra/comment
        QByteArray name = in->read(nameLen);
        if (name.size() != nameLen)
        {
            qCDebug(ZIP_LOG) << "Failed to read name for entry" << i;
            break;
        }
        if (extraLen)
            in->read(extraLen);
        ...
        if (!in->seek(relOffLocal))
        {
            errorString = QStringLiteral("seek LFH failed");
            return false;
        }
        if (rd32(in) != SIG_LFH)
        {
            errorString = QStringLiteral("bad LFH signature");
            return false;
        }
        rd16(in); // version
        rd16(in); // flags (redundant)
        const quint16 lfhMethod = rd16(in);
        Q_UNUSED(lfhMethod);
        rd16(in);
        rd16(in); // time/date
        rd32(in); // crc
        ...
        {
            errorString = outFile.errorString();
            return false;
        }
        ...
        if (method == 0)
        {
            ...
            if (!in->seek(dataPos))
            {
                errorString = QStringLiteral("seek data failed");
                return false;
            }
            ...
            while (left > 0)
            {
                ...
                if (n <= 0)
                {
                    errorString = QStringLiteral("unexpected EOF in stored entry");
                    return false;
                }
                if (outFile.write(buf.constData(), n) != n)
                {
                    errorString = QStringLiteral("short write");
                    return false;
                }
                left -= n;
            }
        ...
        else if (method == 8)
        {
            ...
            {
                errorString = QStringLiteral("inflate init failed");
                return false;
            }
            ...
            while (left > 0 && ret != Z_STREAM_END)
            {
                ...
                if (n <= 0)
                {
                    errorString = QStringLiteral("unexpected EOF in deflate entry");
                    inflateEnd(&strm);
                    return false;
                }
                left -= n;
                ...
                do
                {
                    ...
                    {
                        errorString = QStringLiteral("inflate failed");
                        inflateEnd(&strm);
                        return false;
                    }
                    ...
                    if (have > 0)
                    {
                        if (outFile.write(outChunk.constData(), have) != have)
                        {
                            errorString = QStringLiteral("short write");
                            inflateEnd(&strm);
                            return false;
    ...
    qCDebug(ZIP_LOG) << "Successfully processed" << validEntries << "out of" << totalEntries << "entries";
